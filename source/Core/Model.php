...
abstract class Model
{
    ...
    public function fetch(bool $all = false)
    {
        try {
            $stmt = Connect::getInstance()->prepare(
                $this->statement.$this->join.$this->where.$this->group.$this->order.$this->limit.$this->offset
            );
    ...
    public function update(array $data, string $terms, string $params): ?int
    {
        try {
            ...
            $stmt->execute(array_merge($params, $this->filter($data)));
     ...
     * @return array
     */
    abstract protected static function filter(array $data): array;
}
